    ...
    <div className="space-y-10 md:space-y-12 pb-16">
      ...
      <Tabs defaultValue="overview" className="w-full">
        ...
        <TabsContent value="overview">
          {isLoadingTrip || isLoadingExpenses || isLoadingMembers || isLoadingRecordedPayments ? <Loader2 className="mx-auto h-12 w-12 animate-spin text-primary mt-12" /> :
            errorTrip ? <p className="text-destructive text-center mt-12 text-lg">Error loading overview: {errorTrip.message}</p> :
              trip ? <TripOverviewTab trip={trip} expenses={expenses} members={members} currentUser={currentUser} recordedPayments={recordedPayments} /> : <p className="text-center mt-12 text-lg">No trip data available for overview.</p>}
        </TabsContent>
